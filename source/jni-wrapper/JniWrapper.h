...
{
	// See the documentation of these functions for more details on their
	// purpose and operation. Calling any one of them will start the JVM.

	friend std::string js2ss(jstring javaString);
	friend jstring ss2js(const std::string& stdString);
	friend jclass toClass(const std::string& classPath);
	...
	JavaVM* _javaVirtualMachine = nullptr;

	/**
	 * Just what the name says it is.
	 *
	 * This item is created in the constructor and accessible through the
	 * jniNativeInterface() method.
	 */

	JNIEnv* _jniNativeInterface = nullptr;

	/**
	 * The constructor is declared here to enforce the use of the instance()
	 * method (via the friend functions).
	 */

	JniWrapper();

	/**
	 * In addition to its normal function, the destructor shuts down the
	 * JVM.
	 */
	...
	/**
	 * Create (if it does not already exist) the one and only instance of an
	 * object of this type and return a reference to it.
	 *
	...
	// toClass(), toString(), methodID(), and jniEnv() functions,
	// respectively.

	/** <A NAME="_STRINGTOSTDSTRING_"></A>
	 * Return the std::string representation of the specified Java String
	 * object.
	 *
	 ...
	 * Return a Java String object containing the specified std::string.
	 *
	...
	/** <A NAME="_CLASSPATHTOCLASSOBJ_"></A>
	 * Return the Java Class object associated with the specified class path
	 * (e.g., "java/lang/String").
	 *
	 * The return value is used in the generation of method IDs, as well as
	 * in the calls to Java methods.
	 *
	 * See <A HREF="#_TOCLASS_">toClass</A>() for more details.
	 */

	jclass classPathToClassObj(const std::string& classPath) const noexcept;

	/** <A NAME="_RETURNSTRINGRESULT_"></A>
	 * Return the result of a call to the indicated method in the specified
	 * object, which is expected to return a Java String.
	 *
	 ...
	 * Return a reference to the method with the specified attributes in the
	 * indicated class.
	 *
	 ...
	 *
	 * This provides access to the methods for finding and interacting with
...
// For all of the friend functions defined below, if the JniWrapper object does
// not yet exist, it is created. The caller can't create it explicitly anyway.
 ...
 *
 * A NULL argument will result in an empty return value.
 */
...
/** <A NAME="_SS2JS_"></A>
 * \brief Return a Java String object containing the text within the specified
 * <A HREF="http://en.cppreference.com/w/cpp/string/basic_string">std::string</A>.
 *
 * The returned object should <B>NOT</B> be explicitly deleted, as this task will be
 * performed by the garbage collector of the JVM. An empty argument will result
 * in a matching return value.
 */
...
// This function calls the <A HREF="#_CLASSPATHTOCLASSOBJ_">classPathToClassObj()</A> method of the JniWrapper.

/** <A NAME="_TOCLASS_"></A>
 * \brief Return the Java Class object associated with the specified class path
 * (e.g., "java/lang/String").
 *
 * This function is intended for use in generating an item of this type and to
 * support the caching of it (by the caller). An empty string or any failure in
 * the call will stop the program (via assertion).
 *
 * Note that according to this <A HREF="https://stackoverflow.com/questions/2093111/why-i-should-not-reuse-a-jclass-and-or-jmethodid-in-jni">post</A>, caching a jclass reference may result in
 * unexpected behavior (e.g., segfault) due to garbage collection. Callers
 * should follow the recommended implementation (creating a global reference for
 * the return value).
 */

inline jclass toClass(const std::string& classPath)
{
	return JniWrapper::instance().classPathToClassObj(classPath);
}

// This function calls the <A HREF="#_RETURNSTRINGRESULT_">returnStringResult</A>() method of the JniWrapper.
 ...
 *
 * This function performs the operation described above and converts the Java
 * String into the return type. NULL arguments or any failure in the call will
 * stop the program (via assertion).
 */
...
// This function calls the <A HREF="#_CREATEMETHODID_">createMethodID</A>() method of the JniWrapper.

/** <A NAME="_METHODID_"></A>
 * \brief Return a reference to the method with the specified attributes in the
 * indicated class.
 *
 * This function is intended for use in the most general cases of generating an
 * item of this type and to support the caching of it (by the caller). The
 * return value is NULL if any of the arguments are or a failure occurs in
 * generating the method ID item.
 *
 * Note that the use of const char* instead of std::string for the name and
