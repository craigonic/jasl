...
#             methods to convert a native string to or from a Java <A HREF="http://docs.oracle.com/javase/10/docs/api/java/lang/String.html">String</A>.     #
#                                                                              #
...
my $subStackTestStack = new_Stack_S UiData::Stack($unitTestStack);

# Confirm that a Stack managing a Unit doesn't have sub-stacks.

if (defined $unitTestStack->subStacks())
{
...
if (defined $subStackTestStack->portagedItems())
{
...
my %portagedItems = %{$unitList[2]->portagedItems()};

my @portagedItemKeys = sort(keys %portagedItems);
...
my $newStack = new_Stack_S UiData::Stack($player->takeStack(0));

@unitList = @{$player->stackList()};
my $i = 0;
my $stackListSize = scalar(@unitList);
...
my %subStacks = %{$temporaryStack->subStacks()};

my @subStackKeys = sort(keys %subStacks);

printf("\nThe individual Stacks within the combined one are:\n\n");

foreach my $key (@subStackKeys)
{
    printf("\tKey: %s\t%s\n",
...
my $lmg = CniWrapper::cc2js("3-8 LMG");
